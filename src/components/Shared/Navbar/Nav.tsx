        ...
        <div className={`transition-all ${navBg ? "bg-white shadow-md" : "fixed"} duration-200 h-[12vh] z-[100] fixed w-full`}>
            <div className="flex items-center justify-between w-[90%] xl:w-[80%] h-full mx-auto">
                <div className="flex items-center space-x-2">
                    ...
                    <h1 className="text-xl hidden sm:block md:text-2xl text-black font-bold">BD Food</h1>
                </div>
                <div className='hidden lg:flex items-center space-x-10'>
                    {NavLinks.map((link) => (
                        <Link
                            key={link.id}
                            href={link.url}
                            className='text-black hover:text-green-700 transition-all font-bold duration-200'
                        >
                            <p>{link.Label}</p>
                        </Link>
                    ))}
                ...
                <div className='flex items-center space-x-4'>
                    ...
                    <button className='bg-blue-950 py-2.5 px-8 text-white font-bold rounded-lg hover:bg-black transition-all duration-300 cursor-pointer'>Join Now</button>
                    <HiBars3BottomRight 
                    onClick={navOpen}
